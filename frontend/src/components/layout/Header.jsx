...
export default function Header({ activeTab, onTabChange, onProfileClick }) {
  return (
    <header className="sticky top-0 z-50 backdrop-blur-md bg-white/70 border-b border-agentx-border">
      <div className="px-3 sm:px-6 py-3 flex items-center justify-between gap-3 sm:gap-6 max-w-[1600px] mx-auto">
        {/* Left: Logo + Title — always visible now */}
        <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-agentx-teal flex items-center justify-center">
            <Shield className="w-5 h-5 sm:w-6 sm:h-6 text-white" strokeWidth={2.5} />
          </div>
          <div>
            <h1 className="text-base sm:text-lg font-bold leading-none">AgentX</h1>
            <p className="text-[10px] text-agentx-muted mt-0.5">AI-Powered PC Monitoring</p>
          </div>
        </div>
            ...
            return (
              <button key={id} onClick={() => onTabChange(id)}
                className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                  isActive ? "bg-white shadow-sm border border-agentx-border text-agentx-text"
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
          ...
          <button onClick={onProfileClick} aria-label="Open profile"
            className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-agentx-bg border border-agentx-border flex items-center justify-center hover:bg-white hover:border-agentx-teal/40 transition">
            <User className="w-4 h-4 sm:w-5 sm:h-5 text-agentx-muted" />
          </button>
        </div>
      </div>

      {/* Mobile tab bar — horizontally scrollable */}
      <nav className="md:hidden flex overflow-x-auto no-scrollbar gap-1 px-3 py-2 border-t border-agentx-border">
        {TABS.map(({ id, label, icon: Icon }) => {
          const isActive = activeTab === id;
          return (
            <button key={id} onClick={() => onTabChange(id)}
              className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap ${
                isActive ? "bg-white shadow-sm border border-agentx-border" : "text-agentx-muted"}`}>
